...
#include<thread>
#include<ShlObj.h>
#include"D:\computer learning\data.h"
#include"D:\computer learning\Time.h"
...
void Func1_FileMaker() {
	...
	if (_access(pathf, 0) == 0) {
		printf("Info:您选择的文件已存在，请问是否要覆盖这个文件？按1确定，按0结束。");
		while (1) {
			switch (_getch()) {
			case '1':break;
			case '0':return;
...
void Func8_HelpInfo() {
	const char* p = "使用说明：\n"
		...
		"版本：2.0 2021/10/30";
	MessageBoxA(NULL, p, "使用说明", MB_OK | MB_ICONINFORMATION | MB_SETFOREGROUND);
	return;
}
